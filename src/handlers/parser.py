...
def parse_config(config_file:str)->argparse.Namespace:
    ...
    parser.add_argument('--data_path', type=str,default=config["data_path"], help='path to the input data_path')
    ...
    parser.add_argument('--num_feature', type=int, default=config["num_feature"], help='number_of_features')

    parser.add_argument('--do_svm', type=bool, default= config["do_svm"], help='do_svm')
    parser.add_argument('--max_iter', type=int, default=config["max_iter"], help='max_iter')

    parser.add_argument('--do_knn', type=bool, default= config["do_knn"], help='do_knn')
    parser.add_argument('--n_neighbors', type=int, default=config["n_neighbors"], help='n_neighbors')

    parser.add_argument('--do_random_forest', type=bool, default= config["do_random_forest"], help='do_random_forest')
    parser.add_argument('--n_estimators', type=int, default=config["n_estimators"], help='n_estimators')
    parser.add_argument('--max_depth', type=lambda x: int(x) if x.isdigit() else None, default=config["max_depth"], help='max_depth')

    parser.add_argument('--do_mlp', type=bool, default= config["do_mlp"], help='do_mlp')
    parser.add_argument('--hidden_layer_sizes', type=Tuple[int], default=config["hidden_layer_sizes"], help='hidden_layer_sizes')
    parser.add_argument('--activation', type=str, default=config["activation"], help='activation')
    parser.add_argument('--learning_rate_init', type=float, default=config["learning_rate_init"], help='learning_rate_init')
    parser.add_argument('--validation_fraction', type=float, default=config["validation_fraction"], help='validation_fraction')
    
    parser.add_argument('--do_logistic_regression', type=bool, default= config["do_logistic_regression"], help='do_logistic_regression')

    parser.add_argument('--do_cnn', type=bool, default= config["do_cnn"], help='do_cnn')
    parser.add_argument('--batch_size_cnn', type=int, default=config["batch_size_cnn"], help='batch_size_cnn')
