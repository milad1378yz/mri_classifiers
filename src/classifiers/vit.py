...
class VisionTransformerClassifier(nn.Module):
    ...
    def trainer(self, train_data, train_label, val_data, val_label, classes, batch_size=32, epochs=40):
        ...
        for epoch in range(epochs):
            ...
            train_acc = self.compute_accuracy(torch.tensor(train_predictions), torch.tensor(train_label))
            print(f"Training Accuracy: {train_acc:.2f}")

            # Calculate validation accuracy
            val_predictions = self.my_predict(val_data, val_label)
            val_acc = self.compute_accuracy(torch.tensor(val_predictions), torch.tensor(val_label))
