...
class NaiveBayes_Classifier:
    ...
    def train(self, train_data, train_label, classes):
        ...
        plt.ylabel("Actual")
        plt.savefig("results/train_confusion_matrix_naive_bayes.png")
        # save the report
        report = classification_report(train_label, predict_train, output_dict=True)
        df = pd.DataFrame(report).transpose()
        df.to_csv("results/train_classification_report_naive_bayes.csv")

    def val(self, val_data, val_label, classes):
        # predict
        predict_val = self.clf.predict(val_data)
        # accuracy
        print("validation accuracy (Naive Bayes): ", accuracy_score(val_label, predict_val))
        # classification report
        ...
        print("validation confusion matrix (Naive Bayes): \n", cm)
        # plot confusion matrix
        df_cm = pd.DataFrame(cm, index=classes, columns=classes)
        plt.figure(figsize=(10, 7))
        sns.heatmap(df_cm, annot=True, cmap='Blues', fmt='g')
        plt.xlabel("Predicted")
        plt.ylabel("Actual")
        plt.savefig("results/validation_confusion_matrix_naive_bayes.png")
        # save the report
        report = classification_report(val_label, predict_val, output_dict=True)
        df = pd.DataFrame(report).transpose()
        df.to_csv("results/validation_classification_report_naive_bayes.csv")
